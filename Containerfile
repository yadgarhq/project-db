...
# digest in every Containerfile would mean editing every one of them each time a
# base is rebuilt for a CVE — which is how bases stop being rebuilt. The base
# images are themselves built, scanned, signed and digest-pinned by the
# base-images workflow (D61, D63); this is where that indirection is spent.
#
...
COPY --from=builder /app/target/x86_64-unknown-linux-musl/release/yadgar-project-db /yadgar-project-db
