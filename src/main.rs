...
use yadgar_project_db::pb::yadgar::project::v1::project_db_service_server::ProjectDbServiceServer;
use yadgar_project_db::{boot, rotate, schema, service::ProjectDb};

/// What this module needs of its engine (D69). Addressed, not ranked — so no
/// vector search and no full-text (D10). A registry is looked up by path and by
/// prefix; requiring either would make this module refuse to boot on an engine
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        ...
        // capability probe result and its errors all vanish. A service nobody
    ...
    // `SslModeCannotVerify { .. }` on the operator's terminal instead of the
    ...
    //
    // NO COMPILED-IN PATH BEHIND IT ANY MORE (ADR-0569). `chart/templates/
    // deployment.yaml` renders `DB_PASSWORD_FILE`, and the directory half of that
    // value is the `db-password` volume mount beside it — one path written twice,
    // in one file, rather than a third copy sitting here where no operator reading
    // the chart would find it.
    ...
    // THE ROTATION SCHEDULE, FROM THE MOUNTED DOCUMENT AND FROM NOWHERE ELSE
    // (ADR-0569). `yadgarhq/config` renders it into the `shared` ConfigMap,
    ...
    //    here is the whole point of D7. Its own CONNECTION, never its own
    //    connection OPTIONS: the options come from the same function the pool
...
async fn serve(
    ...
    let serving = tokio::spawn(
        server
            .add_service(ProjectDbServiceServer::new(ProjectDb::new(pool)))
    ...
    // without `drain_within` would trade an expired certificate for a pod that
...
mod tests {
    ...
    fn a_set_value_is_returned_verbatim() {
        std::env::set_var("YADGAR_TEST_PROJECT_DB_PRESENT", "0.0.0.0:50051");
        assert_eq!(
            env_required("YADGAR_TEST_PROJECT_DB_PRESENT").as_deref(),
            Ok("0.0.0.0:50051")
        );
    }
    ...
    fn an_absent_knob_refuses_and_names_itself() {
        std::env::remove_var("YADGAR_TEST_PROJECT_DB_ABSENT");
        let err = env_required("YADGAR_TEST_PROJECT_DB_ABSENT").unwrap_err();
        assert!(
            err.contains("YADGAR_TEST_PROJECT_DB_ABSENT"),
    ...
    fn an_empty_knob_refuses_with_its_own_message() {
        std::env::set_var("YADGAR_TEST_PROJECT_DB_EMPTY", "");
        std::env::remove_var("YADGAR_TEST_PROJECT_DB_EMPTY_ABSENT");
        let empty = env_required("YADGAR_TEST_PROJECT_DB_EMPTY").unwrap_err();
        let absent = env_required("YADGAR_TEST_PROJECT_DB_EMPTY_ABSENT").unwrap_err();
        assert!(empty.contains("set but EMPTY"), "got: {empty}");
        assert!(
            empty.replace("YADGAR_TEST_PROJECT_DB_EMPTY", "K")
                != absent.replace("YADGAR_TEST_PROJECT_DB_EMPTY_ABSENT", "K"),
