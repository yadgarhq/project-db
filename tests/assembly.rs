...
//! can make: **an `project-db` configured this way reads exactly these files, so
...
//! [`yadgar_project_db::rotate::watch_set`], the SAME function `main.rs` calls.
//!
...
/// Everything a fully configured `project-db` reads at boot.
///
...
fn mount() -> Mount {
    ...
    ca_params.distinguished_name.push(
        DnType::CommonName,
        "yadgar-project-db assembly test authority",
    );
    let ca = CertifiedIssuer::self_signed(ca_params, ca_key).unwrap();

    let key = KeyPair::generate().unwrap();
    let mut params = CertificateParams::new(vec!["project-db".to_string()]).unwrap();
    params.extended_key_usages = vec![ExtendedKeyUsagePurpose::ServerAuth];
    params.not_after = date_time_ymd(2027, 6, 15);
    params
        .distinguished_name
        .push(DnType::CommonName, "project-db");
...
#[test]
fn a_fully_configured_project_db_watches_every_file_it_read() {
    let mount = mount();
...
#[test]
fn a_cleartext_project_db_still_watches_its_database_password() {
