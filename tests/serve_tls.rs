...
//! stands up this module's own listener through [`yadgar_project_db::boot::server`]
...
fn pki(san: &str) -> Pki {
    ...
    ca_params
        .distinguished_name
        .push(DnType::CommonName, "yadgar-project-db test authority");
...
async fn request(mut channel: Channel) -> Result<u16, String> {
    let req = http::Request::builder()
        ...
        .method("POST")
        .uri(format!(
            "https://{SERVED_NAME}/yadgar.project.v1.ProjectDbService/Probe"
        ))
...
/// THE POINT: a client speaking TLS gets an answer, so the hop that carries the
/// registry can be encrypted at all. HTTP 200 with a gRPC
...
async fn a_certificate_that_cannot_be_read_refuses_the_boot() {
    let p = pki(SERVED_NAME);
    let key = TempPem::with(&p.key_pem);
    let missing = std::env::temp_dir().join("yadgar-project-db-no-such-cert-31c7ae.pem");
...
async fn a_private_key_that_cannot_be_read_refuses_the_boot() {
    let p = pki(SERVED_NAME);
    let cert = TempPem::with(&p.cert_pem);
    let missing = std::env::temp_dir().join("yadgar-project-db-no-such-key-8de402.pem");
