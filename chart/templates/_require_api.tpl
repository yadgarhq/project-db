...
parent renders `platform`, `project-db`, `project-db` and `task-db` together in ONE
...
CALL IT WITH A DICT:
  ...
  {{- include "project-db.require-api" (dict
        "context"    $
        "apiVersion" "k8s.mariadb.com/v1alpha1"
        "operator"   "mariadb-operator"
        "toggle"     "database.create") }}
*/}}
{{- define "project-db.require-api" -}}
{{- $context := .context -}}
{{- if not ($context.Capabilities.APIVersions.Has .apiVersion) -}}
{{- fail (printf (join "" (list
      "project-db: this render needs the API %s, which %s provides, and the target does not have it. "
