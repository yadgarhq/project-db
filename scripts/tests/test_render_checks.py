...
list rather than reconstructing it:
  ...
  - the template name is `project-db.require-api`, not `platform.require-api`. Helm
    template names are GLOBAL across a chart tree, and at the plan's step 9 the
    parent renders `platform`, `project-db`, `project-db` and `task-db` in ONE namespace
