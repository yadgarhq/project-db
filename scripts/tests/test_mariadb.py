...
WHAT MOVED AND WHY IT MOVED HERE. `yadgarhq/deploy`'s `infra/databases/project-db.yaml`
...
MariaDB CRs belong to `project-db`, `project-db` and `task-db` behind `database.create`."
...
a `yaml.dump(...)` substring search answers "the text `project-db-mariadb` appears
...
`deploy/infra/databases/project-db.yaml`'s "THE NAMES ARE A CONTRACT WITH THE CONSUMER"
...
def mounted_password_secret(deployment: dict) -> tuple[str, str]:
    ...
    `deploy/infra/databases/project-db.yaml` records having met.
    """
    (container,) = deployment["spec"]["template"]["spec"]["containers"]
